...
class Hierachy_Sparse_Model(Attention_Model):
                    ...
                    if encoder_output is not None:
                        with tf.variable_scope("encdec_attention_top"):
                            y = ATT.multihead_attention_cross(
                                self.layer_preprocess(x),
                                encoder_output["encoder_output"],
                                encoder_output["encoder_top_output"][:,0,:] * encoder_output["encoder_audio_output"][:,0,:],
                                encoder_decoder_attention_bias,
                                self.dim_model,
                                self.dim_model,
                                self.dim_model,
                        ...
                        with tf.variable_scope("encdec_attention"):
                            y = ATT.multihead_attention_cross(
                                self.layer_preprocess(x_old),
                                encoder_output["encoder_top_output"],
                                encoder_output["encoder_output"][:,0,:] * encoder_output["encoder_audio_output"][:,0,:],
                                encoder_decoder_pool_attention_bias,
                                self.dim_model,
                                self.dim_model,
                                self.dim_model,
                        ...
                        with tf.variable_scope("encdec_audio"):
                            y = ATT.multihead_attention_cross(
                                self.layer_preprocess(x_old2),
                                encoder_output["encoder_audio_output"],
                                encoder_output["encoder_output"][:,0,:] * encoder_output["encoder_top_output"][:,0,:],
                                encoder_decoder_audio_attention_bias,
                                self.dim_model,
                                self.dim_model,
                                self.dim_model,
    ...
    def build_train_model(self):
        ...
        with tf.name_scope("distilled_loss"):
            ...
            global_image = self.encoder_output["encoder_output"][:,0,:]
            global_motion = self.encoder_output["encoder_top_output"][:,0,:]
            global_audio = self.encoder_output["encoder_audio_output"][:,0,:]
