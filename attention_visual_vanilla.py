...
class Hierachy_Sparse_Model(Attention_Model):
    ...
    def encode(self, inputs, num_encode_layers=0, name="encoder"):
        encoder_input, self_attention_bias, encoder_decoder_attention_bias = (
            self.model_prepare_encoder(inputs))
        encoder_output, ww = self.model_encoder(
            encoder_input, 
            self_attention_bias,
            num_encode_layers=num_encode_layers,
            name=name)
        return encoder_output, encoder_decoder_attention_bias, ww
    ...
    def encode2(self, input_image, input_motion, num_encode_layers=0, name="encoder"):
        ...
        encoder_output_image, encoder_output_motion, temp_ori, temp_index = self.model_encoder_boundary(
            encoder_input_image,
            ...
            num_encode_layers=num_encode_layers,
            name=name)
        return encoder_output_image, encoder_output_motion, encoder_decoder_attention_bias_image, encoder_decoder_attention_bias_motion, temp_ori, temp_index
    
    
    def decode(self, decoder_input, encoder_output, encoder_decoder_attention_bias, encoder_decoder_pool_attention_bias):
        decoder_input, decoder_self_attention_bias = self.model_prepare_decoder(decoder_input)
        decoder_output, ww = self.model_decoder(
            decoder_input,
            encoder_output,
            decoder_self_attention_bias,
            encoder_decoder_attention_bias,
            encoder_decoder_pool_attention_bias)
        return decoder_output, ww
            ...
            for layer in xrange(num_encode_layers):
                with tf.variable_scope("layer_%d" % layer):
                    with tf.variable_scope("self_attention"):
                        ...
                        y,ww = ATT.multihead_attention_vanilla(
                            self.layer_preprocess(x),
                            None,
                            encoder_self_attention_bias,
                            self.dim_model,
                    ...
                    with tf.variable_scope("ffn"):
                        y = self.model_ffn_layer(self.layer_preprocess(x))
                        x = self.layer_postprocess(x, y)
        return self.layer_preprocess(x), ori[0]
    ...
    def model_encoder_boundary(self,
        ...
        with tf.variable_scope(name, reuse=reuse):
            for layer in xrange(num_encode_layers):
                with tf.variable_scope("layer_%d" % layer):
                    ...
                    with tf.variable_scope("self_attention_image"):
                        y_image, temp_ori, temp_index = SparseSelfBoundaryAttention_Visual(self.layer_preprocess(x_image), self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        x_image = self.layer_postprocess(x_image, y_image)
        ...
        return self.layer_preprocess(x_image), self.layer_preprocess(x_motion), temp_ori, temp_index
                ...
                with tf.variable_scope("layer_%d" % layer):
                    ...
                    if encoder_output is not None:
                        with tf.variable_scope("encdec_attention_top"):
                            ...
                            td_ori = tf.reshape(logits_image, [batch_size, -1, current_t, image_len])[:,:,-1,:]
                            td_ori = tf.nn.softmax(td_ori, -1)
                            td_index = tf.reshape(grad_image_index, [batch_size, -1, current_t, image_len//step])[:,:,-1,:]
                    ...
                    with tf.variable_scope("ffn"):
                        y = self.model_ffn_layer(self.layer_preprocess(x))
                        x = self.layer_postprocess(x, y)
        return self.layer_preprocess(x), td_ori, td_index
    ...
    def model_fn(self, inputs, inputs_top, targets):
        encoder_output = {}
        encoder_image_output, encoder_decoder_attention_bias, image_ww = self.encode(inputs, num_encode_layers=self.encoder_layers)
        encoder_top_output, encoder_decoder_top_attention_bias, motion_ww = self.encode(inputs_top, num_encode_layers=self.encoder_layers, name="encode_top")
        ...
        decoder_output, decoder_ww = self.decode(targets, encoder_output, encoder_decoder_attention_bias, encoder_decoder_top_attention_bias)
        return encoder_output, decoder_output, image_ww, decoder_ww
    ...
    def model_fn2(self, inputs, inputs_top, targets):
        ...
        encoder_image_output, encoder_top_output, encoder_decoder_attention_bias, encoder_decoder_top_attention_bias, temp_ori, temp_index = self.encode2(inputs, inputs_top, num_encode_layers=self.encoder_layers)
        ...
        decoder_output, td_ori, td_index = self.decode(targets, encoder_output, encoder_decoder_attention_bias, encoder_decoder_top_attention_bias)
        return encoder_output, decoder_output, temp_ori, temp_index, td_ori, td_index
    ...
    def build_train_model(self):
        ...
        self.encoder_output, self.decoder_output,_,_ = self.model_fn(self.image_emb, self.top_emb, self.sent_emb)
        self.decoder_output = tf.reshape(self.decoder_output, [-1, self.dim_model])
        self.logits = tf.nn.xw_plus_b(self.decoder_output, self.softmax_W, self.softmax_b)
        self.logits = tf.reshape(self.logits, [self.batch_size, self.n_words, self.vocab_size])
    ...
    def greedy_decode(self, decode_length):
        ...
        def symbols_to_logits_fn(ids):
            ...
            with tf.variable_scope("word_emb_linear"):
                ...
                targets = tf.nn.xw_plus_b(targets, self.W_word, self.b_word)
                targets = tf.reshape(targets, [self.batch_size, -1, self.dim_model])
            _, decode_output, image_ww, decoder_ww = self.model_fn(self.image_emb, self.top_emb, targets)
            decode_output = tf.reshape(decode_output, [-1, self.dim_model])
            logits = tf.nn.xw_plus_b(decode_output, self.softmax_W, self.softmax_b)
            logits = tf.reshape(logits, [self.batch_size, -1, self.vocab_size])
            return logits, image_ww, decoder_ww 
        
        def inner_loop(i, decoded_ids, logits, image_ww, decoder_ww):
            logits, image_ww, decoder_ww_ = symbols_to_logits_fn(decoded_ids)
            next_id = tf.expand_dims(tf.argmax(logits[:,-1], axis=-1), axis=1)
            decoded_ids = tf.concat([decoded_ids, next_id], axis=1)
            
            decoder_ww = tf.concat([decoder_ww, tf.expand_dims(decoder_ww_, 2)], 2)
            
            
            
            return i+1, decoded_ids, logits, image_ww, decoder_ww
        
        decoded_ids = tf.zeros([self.batch_size, 1], dtype=tf.int64)
        image_ww = tf.zeros([self.batch_size, 8, 80, 80])
        #temp_index = tf.zeros([self.batch_size, 8, 80, 20])
        decoder_ww = tf.zeros([self.batch_size, 8, 1, 80])
        
        
        _, decoded_ids, logits, image_ww, decoder_ww = tf.while_loop(
            lambda i, *_: tf.less(i, decode_length),
            inner_loop,
            [tf.constant(0), decoded_ids, tf.zeros([self.batch_size,1,1]), image_ww, decoder_ww],
            shape_invariants=[
                tf.TensorShape([]),
                tf.TensorShape([None, None]),
                tf.TensorShape([None, None, None]),
        ...
        return decoded_ids, logits, image_ww, decoder_ww
