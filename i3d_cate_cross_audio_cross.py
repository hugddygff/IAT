...
import multi_view_attention_model_audio_cross_super
...
model_path = '/data1/jintao/msrvtt_global_local/train_model/i3d_cate_audio_cross3'
...
def train():
    ...
    with g_train.as_default():
        model = multi_view_attention_model_audio_cross_super.Hierachy_Sparse_Model(
    ...
    with g_valid.as_default():
        model_valid = multi_view_attention_model_audio_cross_super.Hierachy_Sparse_Model(
                #batch_size=batch_size,
                batch_size = 16,
                n_frames=n_frames,
                dim_image=dim_image + 600,
    ...
    loss_fd = open('./result_txt/i3d_cate_audio_cross3.txt', 'w')
    ...
    for epoch in range(0, n_epochs):
        ...
        saver.save(sess, os.path.join(model_path, 'i3d_cate_audio_cross'), global_step=epoch)
        
        if epoch > 50:
        
            saver_valid.restore(sess_valid,'/data1/jintao/msrvtt_global_local/train_model/i3d_cate_audio_cross3/i3d_cate_audio_cross-' + str(epoch))
...
def test(model_path='/data1/jintao/msrvtt_global_local/train_model/i3d_cate_audio_cross3/i3d_cate_audio_cross-68'):
    test_data = get_video_test_data(video_test_data_path, video_test_feat_path, video_test_3d_path, video_test_mfcc_path, video_test_cate_path, image_pred_test_path, i3d_pred_test_path)
    ...
    with g_test.as_default():
        model_test = multi_view_attention_model_audio_cross_super.Hierachy_Sparse_Model(
                batch_size=batch_size,
                n_frames=n_frames,
                dim_image=dim_image + 600,
                n_audio=n_audio,
    ...
    test_output_txt_fd = open('bi_results_cross3_68.txt', 'w')
