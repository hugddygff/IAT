...
model_path = '/data2/jintao/msrvtt_global_local/train_model/i3d_cate_vanilla'
...
def get_validation_bleu(sess, current_val_data, ixtoword, wordtoix, decoded_ids, model, train_data, temp_ori, temp_index, td_ori, td_index):
    ...
    sbat_gen = open("data/try.txt","w")
    sbat_encode_ori = "data/sbat_encode_local2/"
    sbat_encode_index = "data/sbat_encode_local_index/"
    sbat_decode_ori = "data/sbat_decode_local/"
    sbat_decode_index = "data/sbat_decode_local_index/"
    
    os.mkdir(sbat_encode_ori)
    os.mkdir(sbat_encode_index)
    os.mkdir(sbat_decode_ori)
    os.mkdir(sbat_decode_index)
    ...
    for start, end in zip(
            ...
            generated_word_index, encode_ori, encode_index, decode_ori, decode_index = sess.run([decoded_ids, temp_ori, temp_index, td_ori, td_index], feed_dict={model.video: current_feats,
            ...
            for i in range(batch_size):
                ...
                sbat_gen.write(current_videos[i] + "\n")
                sbat_gen.write(generat_sentence + "\n")
                
                save_name = current_videos[i].split("/")[-1]
                print encode_ori[i].shape
                np.save(sbat_encode_ori + save_name, encode_ori[i])
                print encode_index[i].shape
                np.save(sbat_encode_index + save_name, encode_index[i])
                print decode_ori[i].shape
                np.save(sbat_decode_ori + save_name, decode_ori[i])
                print decode_index[i].shape
                np.save(sbat_decode_index + save_name, decode_index[i])
...
def train():
    ...
    with g_train.as_default():
        model = attention_visual.Hierachy_Sparse_Model(
        ...
        #saver.restore(sess,'/data2/jintao/msrvtt_global_local/train_model/i3d_cate_boundary2/i3d_cate_boundary2-78')
        
        #embed()
        
    g_valid = tf.Graph()
    with g_valid.as_default():
        model_valid = attention_visual.Hierachy_Sparse_Model(
                #batch_size=batch_size,
                batch_size = 20,
                n_frames=n_frames,
                dim_image=dim_image + 600,
        ...
        decoded_ids, _, temp_ori, temp_index, td_ori, td_index = model_valid.greedy_decode(decode_length=n_words)
    ...
    loss_fd = open('./result_txt/i3d_cate_vanilla______.txt', 'w')
    ...
    for epoch in range(0, n_epochs):
        ...
        saver.save(sess, os.path.join(model_path, 'i3d_cate_vanilla'), global_step=epoch)
        '''
        
        
        if epoch > -1:
        
            saver_valid.restore(sess_valid,'/data2/jintao/msrvtt_global_local/train_model/i3d_cate_boundary2/i3d_cate_boundary2-5')
            ...
            val_bleu_epoch = get_validation_bleu(sess_valid,
                        ...
                        ixtoword,wordtoix,
                        decoded_ids, model_valid, val_data, temp_ori, temp_index, td_ori, td_index)
...
def test(model_path='/data2/jintao/msrvtt_global_local/train_model/i3d_cate_boundary2/i3d_cate_boundary2-5'):
    test_data = get_video_test_data(video_test_data_path, video_test_feat_path, video_test_3d_path, video_test_mfcc_path, video_test_cate_path)
    ...
    with g_test.as_default():
        model_test = attention_visual.Hierachy_Sparse_Model(
                batch_size=batch_size,
                n_frames=n_frames,
                dim_image=dim_image + 600,
                n_words=n_words,
    ...
    test_output_txt_fd = open('bi_results5.txt', 'w')
