...
import multi_view_attention_model_audio
...
model_path = '/data1/jintao/msrvtt_global_local/train_model/i3d_cate_restart'
...
def train():
    ...
    with g_train.as_default():
        model = multi_view_attention_model_audio.Hierachy_Sparse_Model(
        ...
        #saver.restore(sess,'/data1/jintao/msrvtt_global_local/train_model/i3d_cate_restart/i3d_cate_restart-50')
        
        #embed()
        
    g_valid = tf.Graph()
    with g_valid.as_default():
        model_valid = multi_view_attention_model_audio.Hierachy_Sparse_Model(
                #batch_size=batch_size,
                batch_size = 20,
                n_frames=n_frames,
                dim_image=dim_image + 600,
    ...
    loss_fd = open('./result_txt/i3d_cate_restart.txt', 'w')
    ...
    for epoch in range(0, n_epochs):
        ...
        saver.save(sess, os.path.join(model_path, 'i3d_cate_restart'), global_step=epoch)
        
        if epoch > 50:
        
            saver_valid.restore(sess_valid,'/data1/jintao/msrvtt_global_local/train_model/i3d_cate_restart/i3d_cate_restart-' + str(epoch))
...
def test(model_path='/data1/jintao/msrvtt_global_local/train_model/i3d_cate_restart/i3d_cate_restart-5'):
    test_data = get_video_test_data(video_test_data_path, video_test_feat_path, video_test_3d_path, video_test_mfcc_path, video_test_cate_path)
    ...
    with g_test.as_default():
        model_test = multi_view_attention_model_audio.Hierachy_Sparse_Model(
                batch_size=batch_size,
                n_frames=n_frames,
                dim_image=dim_image + 600,
                n_audio=n_audio,
    ...
    test_output_txt_fd = open('bi_results_try.txt', 'w')
