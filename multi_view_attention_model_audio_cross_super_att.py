...
class Hierachy_Sparse_Model(Attention_Model):
    def __init__(self, 
        ...
        with tf.variable_scope("W_audio_encode"):
            self.encode_audio_W = tf.Variable(tf.random_normal([428, dim_model], 0.0, dim_model**-0.5), dtype=tf.float32, name="audio_w")
            self.encode_audio_b = tf.Variable(tf.zeros([dim_model]), dtype=tf.float32, name="audio_b")
            
        
        self.decouple_image = tf.Variable(tf.random_normal([1, 1, 4, 4]), dtype=tf.float32, name="decouple_image")
        self.decouple_motion = tf.Variable(tf.random_normal([1, 4, 1, 4]), dtype=tf.float32, name="decouple_motion")
        self.decouple_audio = tf.Variable(tf.random_normal([1, 4, 4, 1]), dtype=tf.float32, name="decouple_audio")
    ...
    def encode2(self, input_image, input_motion, num_encode_layers=0, name="encoder"):
        ...
        encoder_output_image, encoder_output_motion = self.model_encoder_cross(
            encoder_input_image,
    ...
    def encode3(self, input_image, input_motion, input_audio, num_encode_layers=0, name="encoder"):
        encoder_input_image, self_attention_bias_image, encoder_decoder_attention_bias_image = (
            self.model_prepare_encoder(input_image))
        ...
        encoder_input_audio, self_attention_bias_audio, encoder_decoder_attention_bias_audio = (
            self.model_prepare_encoder(input_audio))
        ...
        #encoder_output_audio = self.model_encoder(
        #    encoder_input_audio,
        #    self_attention_bias_audio,
        #    num_encode_layers=num_encode_layers,
        #    name="encoder_audio")
        
        encoder_output_image, encoder_output_motion, encoder_output_audio = self.model_encoder_triple(
            encoder_input_image,
            encoder_input_motion,
            encoder_input_audio,
            self_attention_bias_image,
        ...
        return encoder_output_image, encoder_output_motion, encoder_output_audio, encoder_decoder_attention_bias_image, encoder_decoder_attention_bias_motion, encoder_decoder_attention_bias_audio
    ...
    def decode2(self, decoder_input, encoder_output, encoder_decoder_attention_bias, encoder_decoder_pool_attention_bias, encoder_decoder_audio_attention_bias):
        decoder_input, decoder_self_attention_bias = self.model_prepare_decoder(decoder_input)
        decoder_output = self.model_decoder_td(
            decoder_input,
            encoder_output,
            decoder_self_attention_bias,
            encoder_decoder_attention_bias,
        ...
        with tf.variable_scope(name, reuse=reuse):
            for layer in xrange(num_encode_layers):
                with tf.variable_scope("layer_%d" % layer):
                    ...
                    with tf.variable_scope("self_attention_audio"):
                        #y_audio = SparseSelfAttention_(self.layer_preprocess(x_audio),self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        y_audio = common_attention.multihead_attention(
                            self.layer_preprocess(x_audio),
                            None,
                            encoder_audio_bias,
                            self.dim_model,
                            self.dim_model,
                        ...
                        x_audio = self.layer_postprocess(x_audio, y_audio)
                    with tf.variable_scope("ffn_audio"):
                        y_audio = self.model_ffn_layer(self.layer_preprocess(x_audio))
                        x_audio = self.layer_postprocess(x_audio, y_audio)
                        #x_motion = self.layer_preprocess(x_motion)
                    ...
                    #x_motion[:,0,:]
                    #x_audio[:,0,:]
                    global_image = x_image[:,0,:]
                    global_motion = x_motion[:,0,:]
                    global_audio = x_audio[:,0,:]
                    
                    
                        
                    with tf.variable_scope("cross_attention_image_motion"):
                        y_image_motion = SparseCrossAttention_gate(self.layer_preprocess(x_image), self.layer_preprocess(x_motion), self.layer_preprocess(x_audio)[:,0,:], self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        #cross_image_motion = self.layer_postprocess(x_image, y_image)
                    
                    with tf.variable_scope("cross_attention_image_audio"):
                        #y_image = SparseCrossAttention_(self.layer_preprocess(x_image), self.layer_preprocess(x_motion), self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        
                        y_image_audio = ATT.multihead_attention_cross(
                            self.layer_preprocess(x_image),
                            self.layer_preprocess(x_audio),
                            self.layer_preprocess(x_motion)[:,0,:],
                            encoder_audio_bias,
                            self.dim_model,
                            self.dim_model,
                            self.dim_model,
                        ...
                        cross_image = self.layer_postprocess(x_image, y_image_motion + y_image_audio)
                    
                    
                    with tf.variable_scope("cross_attention_motion_image"):
                        y_motion_image = SparseCrossAttention_gate(self.layer_preprocess(x_motion), self.layer_preprocess(x_image), self.layer_preprocess(x_audio)[:,0,:], self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        #cross_motion = self.layer_postprocess(x_motion, y_motion)
                        
                    
                    
                    with tf.variable_scope("cross_attention_motion_audio"):
                        #y_image = SparseCrossAttention_(self.layer_preprocess(x_image), self.layer_preprocess(x_motion), self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        
                        y_motion_audio = ATT.multihead_attention_cross(
                            self.layer_preprocess(x_motion),
                            self.layer_preprocess(x_audio),
                            self.layer_preprocess(x_image)[:,0,:],
                            encoder_audio_bias,
                            self.dim_model,
                            self.dim_model,
                            self.dim_model,
                        ...
                        cross_motion = self.layer_postprocess(x_motion, y_motion_image + y_motion_audio)
                    
                    
                    with tf.variable_scope("cross_attention_audio_image"):
                        #y_image = SparseCrossAttention_(self.layer_preprocess(x_image), self.layer_preprocess(x_motion), self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        
                        y_audio_image = ATT.multihead_attention_cross(
                            self.layer_preprocess(x_audio),
                            self.layer_preprocess(x_image),
                            self.layer_preprocess(x_motion)[:,0,:],
                            encoder_image_bias,
                        ...
                        #cross_audio_image = self.layer_postprocess(x_audio, y_audio)
                    
                    
                    with tf.variable_scope("cross_attention_audio_motion"):
                        #y_image = SparseCrossAttention_(self.layer_preprocess(x_image), self.layer_preprocess(x_motion), self.num_heads, self.dim_model/self.num_heads, 4, 5)
                        
                        y_audio_motion = ATT.multihead_attention_cross(
                            self.layer_preprocess(x_audio),
                            self.layer_preprocess(x_motion),
                            self.layer_preprocess(x_image)[:,0,:],
                            encoder_motion_bias,
                        ...
                        cross_audio = self.layer_postprocess(x_audio, y_audio_image + y_audio_motion)
                        
                    
                    with tf.variable_scope("cross_ffn"):
                        y_image = self.model_ffn_layer(self.layer_preprocess(cross_image))
                        y_motion = self.model_ffn_layer(self.layer_preprocess(cross_motion))
                        y_audio = self.model_ffn_layer(self.layer_preprocess(cross_audio))
                        
                        x_image = self.layer_postprocess(cross_image, y_image)
                        x_motion = self.layer_postprocess(cross_motion, y_motion)
                        x_audio = self.layer_postprocess(cross_audio, y_audio)
                    
                    image_res.append(tf.expand_dims(self.layer_preprocess(x_image), 1))
                    motion_res.append(tf.expand_dims(self.layer_preprocess(x_motion), 1))
                    audio_res.append(tf.expand_dims(self.layer_preprocess(x_audio), 1))
                    
                    
        #return self.layer_preprocess(x_image), self.layer_preprocess(x_motion), self.layer_preprocess(x_audio)
        return tf.concat(image_res, 1), tf.concat(motion_res, 1), tf.concat(audio_res, 1)
            ...
            for layer in xrange(self.decoder_layers):
                with tf.variable_scope("layer_%d" % layer):
                    ...
                    if encoder_output is not None:
                        ...
                        with tf.variable_scope("encdec_audio"):
                            y = common_attention.multihead_attention(
                                self.layer_preprocess(x_old2),
                                encoder_output["encoder_audio_output"],
                                encoder_decoder_audio_attention_bias,
                                self.dim_model,
                                self.dim_model,
                        ...
                        with tf.variable_scope("layer_attention"):
                            ...
                            x_cap = tf.expand_dims(x_old, 2)
                            x = tf.concat([x_audio, x_top, x, x_cap], 2)
            ...
            for layer in xrange(self.decoder_layers):
                with tf.variable_scope("layer_%d" % layer):
                    ...
                    if encoder_output is not None:
                        ...
                        with tf.variable_scope("multimodal_layer_wise"):
                            
                            image_cls = encoder_output["encoder_output"][:,:,0,:]  #[batch_size, 4, hidden_dim]
                            image_cls2 = tf.expand_dims(tf.expand_dims(image_cls, 2),2)
                            
                            motion_cls = encoder_output["encoder_top_output"][:,:,0,:]
                            motion_cls2 = tf.expand_dims(tf.expand_dims(motion_cls, 2),1)
                            
                            audio_cls = encoder_output["encoder_audio_output"][:,:,0,:]
                            audio_cls2 = tf.expand_dims(tf.expand_dims(audio_cls, 1),1)
                            
                            
                            tensor = tf.reduce_sum(image_cls2 * motion_cls2 * audio_cls2, -1)
                            
                            
                            
                            
                            image_logits = tf.reduce_sum(tensor * self.decouple_image, [2, 3])/22.62741699797
                            motion_logits = tf.reduce_sum(tensor * self.decouple_motion, [1, 3])/22.62741699797
                            audio_logits = tf.reduce_sum(tensor * self.decouple_audio, [1, 2])/22.62741699797
                            
                            image_weights = tf.expand_dims(tf.expand_dims(tf.nn.softmax(image_logits, -1), -1), -1)
                            motion_weights = tf.expand_dims(tf.expand_dims(tf.nn.softmax(motion_logits, -1), -1), -1)
                            audio_weights = tf.expand_dims(tf.expand_dims(tf.nn.softmax(audio_logits, -1), -1), -1)
                            
                            
                            image_out = tf.reduce_sum(image_weights * encoder_output["encoder_output"], 1)
                            motion_out = tf.reduce_sum(motion_weights * encoder_output["encoder_top_output"], 1)
                            audio_out = tf.reduce_sum(audio_weights * encoder_output["encoder_audio_output"], 1)
                            
                            
                            
                            
                            
                            #image_out = tf.reshape(encoder_output["encoder_output"], [batch_size, 4, -1])
                            #motion_out = tf.reshape(encoder_output["encoder_top_output"], [batch_size, 4, -1])
                            #audio_out = tf.reshape(encoder_output["encoder_audio_output"], [batch_size, 4, -1])
                            
                            #image_out = tf.reshape(tf.matmul(image_weights, image_out), [batch_size, current_t, -1, self.dim_model])
                            #motion_out = tf.reshape(tf.matmul(motion_weights, motion_out), [batch_size, current_t, -1, self.dim_model])
                            #audio_out = tf.reshape(tf.matmul(audio_weights, audio_out), [batch_size, current_t, -1, self.dim_model])
                            
                            
                            
                            # x [batch_size, current_t, hidden_dim]
                            #x_cls = tf.expand_dims(tf.expand_dims(tf.expand_dims(x, 2), 2), 2)
                            
                            
                            #tensor = tf.reduce_sum(x_cls * image_cls2 * motion_cls2 * audio_cls2, -1) #[batch_size, current_t, 4, 4, 4]
                            
                            #decouple_image = tf.Variable(tf.random_normal([1, 1, 1, 4, 4]), dtype=tf.float32, name="decouple_image")
                            #decouple_motion = tf.Variable(tf.random_normal([1, 1, 4, 1, 4]), dtype=tf.float32, name="decouple_motion")
                            #decouple_audio = tf.Variable(tf.random_normal([1, 1, 4, 4, 1]), dtype=tf.float32, name="decouple_audio")
                            
                            #image_logits = tf.reduce_sum(tensor * decouple_image, [3, 4])/tf.sqrt(self.dim_model)
                            #motion_logits = tf.reduce_sum(tensor * decouple_motion, [2, 4])/tf.sqrt(self.dim_model)
                            #audio_logits = tf.reduce_sum(tensor * decouple_audio, [2, 3])/tf.sqrt(self.dim_model)
                            
                            #image_weights = tf.nn.softmax(image_logits, -1)  #[batch_size, current_t, 4] * [batch_size, 4, len, dim]
                            #motion_weights = tf.nn.softmax(motion_logits, -1)
                            #audio_weights = tf.nn.softmax(audio_logits, -1)
                        ...
                        with tf.variable_scope("encdec_attention_top"):
                            y = ATT.multihead_attention_cross(
                                self.layer_preprocess(x),
                                image_out,
                                motion_out[:,0,:] * audio_out[:,0,:],
                                encoder_decoder_attention_bias,
                                self.dim_model,
                                self.dim_model,
                                self.dim_model,
                        ...
                        with tf.variable_scope("layer_attention"):
                            x_audio = tf.expand_dims(x_audio, 2)   #[batch_size, current_t, 1, dim_hidden] 
                            x_top = tf.expand_dims(x_top, 2)
                            x = tf.expand_dims(x, 2)
    ...
    def model_fn(self, inputs, inputs_top, inputs_audio, targets):
        ...
        encoder_image_output, encoder_top_output, encoder_audio_output, encoder_decoder_attention_bias, encoder_decoder_top_attention_bias, encoder_decoder_audio_attention_bias = self.encode3(inputs, inputs_top, inputs_audio, num_encode_layers=self.encoder_layers)
        ...
        decoder_output = self.decode2(targets, encoder_output, encoder_decoder_attention_bias, encoder_decoder_top_attention_bias, encoder_decoder_audio_attention_bias)
        return encoder_output, decoder_output
    ...
    def build_inputs(self):
        ...
        self.image_pred = tf.placeholder(tf.float32, [self.batch_size, 1001])
        self.i3d_pred = tf.placeholder(tf.float32, [self.batch_size, 400])
        self.audio_pred = tf.reduce_mean(self.audio, 1)  #[self.batch_size, 128]
    ...
    def build_train_model(self):
        ...
        with tf.name_scope("distilled_loss"):
            #self.encoder_output["encoder_output"]: image
            #self.encoder_output["encoder_top_output"]: motion
            #self.encoder_output["encoder_audio_output"]: audio
            self.image_pred_W = tf.Variable(tf.random_normal([self.dim_model, 1001], 0.0, self.dim_model**-0.5), dtype=tf.float32, name="image_pred_w")
            self.image_pred_b = tf.Variable(tf.zeros([1001]), dtype=tf.float32, name="image_pred_b")
            
            self.i3d_pred_W = tf.Variable(tf.random_normal([self.dim_model, 400], 0.0, self.dim_model**-0.5), dtype=tf.float32, name="i3d_pred_w")
            self.i3d_pred_b = tf.Variable(tf.zeros([400]), dtype=tf.float32, name="i3d_pred_b")
            
            self.audio_pred_W = tf.Variable(tf.random_normal([self.dim_model, 128], 0.0, self.dim_model**-0.5), dtype=tf.float32, name="audio_pred_w")
            self.audio_pred_b = tf.Variable(tf.zeros([128]), dtype=tf.float32, name="audio_pred_b")
            
            global_image = self.encoder_output["encoder_output"][:,-1,0,:]
            global_motion = self.encoder_output["encoder_top_output"][:,-1,0,:]
            global_audio = self.encoder_output["encoder_audio_output"][:,-1, 0,:]
            
            pred_image = tf.nn.xw_plus_b(global_image, self.image_pred_W, self.image_pred_b)
            pred_motion = tf.nn.xw_plus_b(global_motion, self.i3d_pred_W, self.i3d_pred_b)
            pred_audio = tf.nn.xw_plus_b(global_audio, self.audio_pred_W, self.audio_pred_b)
            
            loss_image = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.image_pred, logits=pred_image), 0)
            loss_motion = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.i3d_pred, logits=pred_motion), 0)
            loss_audio = tf.reduce_mean(tf.square(pred_audio - self.audio_pred[:,:128]), 0)
        ...
        with tf.name_scope("reg_loss"):
            reg_loss = self.weight_decay()
            #print reg_loss
        total_loss = loss + reg_loss + 0.1*loss_image + 0.1*loss_motion + 0.1*loss_audio
        #print total_loss
