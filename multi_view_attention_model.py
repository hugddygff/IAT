...
import beam_search
import layers 
from attention_model2 import Attention_Model
...
class Hierachy_Sparse_Model(Attention_Model):
    def __init__(self, 
        ...
        with tf.variable_scope("gate1"):
            ...
            self.gate1_W = tf.Variable(tf.random_normal([dim_model*4, 1], 0.0, dim_model**-0.5), dtype=tf.float32, name="gate1_W")
            self.gate1_b = tf.Variable(tf.zeros([1]), dtype=tf.float32, name="gate1_b")
    ...
    def model_fn(self, inputs, inputs_top, targets):
        ...
        #inputs [batch_size, time_length, hidden_size]
        #encoder_output [batch_size, time_length, hidden_size]
        
        
        temp_res = tf.concat([inputs, inputs_top, encoder_image_output, encoder_top_output], 2)
        ...
        a = tf.zeros([tf.shape(temp_res)[0], 1, tf.shape(temp_res)[2]])
        
        temp4 = tf.concat([a, temp3], 1)
        
        
        temp5 = tf.sigmoid(tf.matmul(tf.reshape(temp4, [-1,tf.shape(temp4)[-1]]), self.gate1_W) + self.gate1_b)
        
        temp5 = tf.reshape(temp5, [tf.shape(temp4)[0], tf.shape(temp4)[1], 1])
        
        t_mask = (temp5 > 0.5)
        t_mask = tf.cast(t_mask, tf.float32)
        
        encoder_image_output = encoder_image_output * t_mask
        encoder_top_output = encoder_top_output * t_mask
