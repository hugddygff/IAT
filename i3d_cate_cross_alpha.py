...
model_path = '/data2/jintao/msrvtt_global_local/train_model/i3d_cate_alpha0'
...
def train():
    ...
    with g_train.as_default():
        model = attention_alpha.Hierachy_Sparse_Model(
        ...
        saver.restore(sess,'/data2/jintao/msrvtt_global_local/train_model/i3d_cate_alpha0/i3d_cate_alpha0-82')
        
        #embed()
        
    g_valid = tf.Graph()
    with g_valid.as_default():
        model_valid = attention_alpha.Hierachy_Sparse_Model(
                #batch_size=batch_size,
                batch_size = 20,
                n_frames=n_frames,
                dim_image=dim_image + 600,
    ...
    loss_fd = open('./result_txt/i3d_cate_alpha0.txt', 'a')
    ...
    for epoch in range(0, n_epochs):
        ...
        saver.save(sess, os.path.join(model_path, 'i3d_cate_alpha0'), global_step=epoch)
        
        if epoch > -1:
        
            saver_valid.restore(sess_valid,'/data2/jintao/msrvtt_global_local/train_model/i3d_cate_alpha0/i3d_cate_alpha0-' + str(epoch))
...
def test(model_path='/data2/jintao/msrvtt_global_local/train_model/i3d_cate_alpha0/i3d_cate_alpha0-8'):
    test_data = get_video_test_data(video_test_data_path, video_test_feat_path, video_test_3d_path, video_test_mfcc_path, video_test_cate_path)
    ...
    with g_test.as_default():
        model_test = attention_alpha.Hierachy_Sparse_Model(
                batch_size=batch_size,
                n_frames=n_frames,
                dim_image=dim_image + 600,
                n_words=n_words,
    ...
    test_output_txt_fd = open('a0_results3.txt', 'w')
