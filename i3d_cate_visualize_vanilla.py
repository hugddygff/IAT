...
import attention_visual_vanilla
...
def get_validation_bleu(sess, current_val_data, ixtoword, wordtoix, decoded_ids, model, train_data, image_ww, decoder_ww):
    ...
    diss = {}
    a = {}
    vanilla_gen = open("data/vanilla_generate.txt","w")
    vanilla_encode_ori = "data/vanilla_encode_ori2/"
    
    vanilla_decode_ori = "data/vanilla_decode_ori2/"
    
    os.mkdir(vanilla_encode_ori)
    os.mkdir(vanilla_decode_ori)
    ...
    for start, end in zip(
            ...
            generated_word_index, encode_ori, decode_ori = sess.run([decoded_ids, image_ww, decoder_ww], feed_dict={model.video: current_feats,
            ...
            for i in range(batch_size):
                ...
                vanilla_gen.write(current_videos[i] + "\n")
                vanilla_gen.write(generat_sentence + "\n")
                
                save_name = current_videos[i].split("/")[-1]
                print encode_ori[i].shape
                np.save(vanilla_encode_ori + save_name, encode_ori[i])
                
                print decode_ori[i].shape
                np.save(vanilla_decode_ori + save_name, decode_ori[i])
...
def train():
    ...
    with g_train.as_default():
        model = attention_visual_vanilla.Hierachy_Sparse_Model(
    ...
    with g_valid.as_default():
        model_valid = attention_visual_vanilla.Hierachy_Sparse_Model(
                #batch_size=batch_size,
                batch_size = 20,
                n_frames=n_frames,
                dim_image=dim_image + 600,
        ...
        decoded_ids, _, image_ww, decoder_ww  = model_valid.greedy_decode(decode_length=n_words)
    ...
    for epoch in range(0, n_epochs):
        ...
        if epoch > -1:
            ...
            saver_valid.restore(sess_valid,'/data2/jintao/msrvtt_global_local/train_model/i3d_cate_vanilla/i3d_cate_vanilla-81')
            ...
            val_bleu_epoch = get_validation_bleu(sess_valid,
                        ...
                        ixtoword,wordtoix,
                        decoded_ids, model_valid, val_data, image_ww, decoder_ww)
