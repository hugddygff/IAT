...
def dot_product_attention_vanilla(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
...
def multihead_attention_cross(query_antecedent,
                        memory_antecedent,
                        cross_global,
                        bias,
                        total_key_depth,
  ...
  with tf.variable_scope(
    ...
    cross_global_ = tf.expand_dims(cross_global, 1)
    k = k * cross_global_
...
def multihead_attention_vanilla(query_antecedent,
                        memory_antecedent,
                        bias,
                        total_key_depth,
                        total_value_depth,
  ...
  with tf.variable_scope(
    ...
    if callable(attention_type):  # Generic way to extend multihead_attention
      ...
      x, ww = dot_product_attention_vanilla(q, k, v, bias, dropout_rate, image_shapes)
    elif attention_type == "dot_product_relative":
      x = dot_product_attention_relative(q, k, v, bias, max_relative_position,
                                         dropout_rate, image_shapes)
    elif attention_type == "local_mask_right":
...
def multihead_attention_weights(query_antecedent,
                        memory_antecedent,
                        bias,
                        total_key_depth,
                        total_value_depth,
  ...
  with tf.variable_scope(
    ...
    if callable(attention_type):  # Generic way to extend multihead_attention
      ...
      weights = dot_product_weights(q, k, v, bias, dropout_rate, image_shapes)
    elif attention_type == "dot_product_relative":
      x = dot_product_attention_relative(q, k, v, bias, max_relative_position,
                                         dropout_rate, image_shapes)
    elif attention_type == "local_mask_right":
...
def multihead_attention_logits(query_antecedent,
                        memory_antecedent,
                        bias,
                        total_key_depth,
                        total_value_depth,
  ...
  with tf.variable_scope(
    ...
    if callable(attention_type):  # Generic way to extend multihead_attention
      ...
      logits = dot_product_logits(q, k, v, bias, dropout_rate, image_shapes)
    elif attention_type == "dot_product_relative":
      x = dot_product_attention_relative(q, k, v, bias, max_relative_position,
                                         dropout_rate, image_shapes)
    elif attention_type == "local_mask_right":
