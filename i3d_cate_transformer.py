...
import multi_view_attention_model
...
model_path = '/data2/jintao/msrvtt_global_local/train_model/i3d_cate_sparse_transformer'
...
def train():
    ...
    with g_train.as_default():
        model = multi_view_attention_model.Hierachy_Sparse_Model(
        ...
        #saver.restore(sess,'/data2/jintao/msrvtt_global_local/train_model/i3d_cate_transformer/i3d_cate_transformer-91')
        
        #embed()
        
    g_valid = tf.Graph()
    with g_valid.as_default():
        model_valid = multi_view_attention_model.Hierachy_Sparse_Model(
                batch_size=batch_size,
                n_frames=n_frames,
                dim_image=dim_image + 600,
                n_words=n_words,
    ...
    loss_fd = open('./result_txt/i3d_cate_sparse_transformer.txt', 'a')
    ...
    for epoch in range(0, n_epochs):
        ...
        saver.save(sess, os.path.join(model_path, 'i3d_cate_sparse_transformer'), global_step=epoch)
        
        saver_valid.restore(sess_valid,'/data2/jintao/msrvtt_global_local/train_model/i3d_cate_sparse_transformer/i3d_cate_sparse_transformer-' + str(epoch))
        ...
        #if val_bleu_epoch > best_cider:
        #    best_cider = val_bleu_epoch
        #    lr_flag = 0
        #else:
        #    lr_flag += 1
        #    if lr_flag == 5:
...
def test(model_path='/data2/jintao/msrvtt_global_local/train_model/i3d_cate_cross_transformer_decoder/i3d_cate_cross_transformer_decoder-95'):
    test_data = get_video_test_data(video_test_data_path, video_test_feat_path, video_test_3d_path, video_test_mfcc_path, video_test_cate_path)
    ...
    with g_test.as_default():
        model_test = multi_view_attention_model.Hierachy_Sparse_Model(
                batch_size=1,
                n_frames=n_frames,
                dim_image=dim_image + 600,
                n_words=n_words,
    ...
    test_output_txt_fd = open('bi_results6.txt', 'w')
    for idx, video_feat_path in enumerate(concate_videos):
        ...
        if video_feat.shape[1] == n_frames:
            video_mask = np.ones((video_feat.shape[0], video_feat.shape[1]))
            video_feat = np.concatenate([video_cate_feat, video_feat, video_cate_feat], 2)
            
        else:
            shape_templete = np.zeros(shape=(1, n_frames, dim_image), dtype=float )
            shape_templete[:video_feat.shape[0], :video_feat.shape[1], :video_feat.shape[2]] = video_feat
            video_feat = shape_templete
            
            video_feat = np.concatenate([video_cate_feat, video_feat, video_cate_feat], 2)
            
            video_mask = np.zeros((video_feat.shape[0], n_frames))
        ...
        generated_word_index = sess_test.run(
                            decoded_ids,
                            feed_dict={
                            model_test.video: video_feat,
                            model_test.video_mask : video_mask
                            })
        #embed()
...
def test2(model_path='/data2/jintao/msrvtt_global_local/train_model/i3d_cate_sparse_transformer/i3d_cate_sparse_transformer-84'):
    test_data = get_video_test_data(video_test_data_path, video_test_feat_path, video_test_3d_path, video_test_mfcc_path, video_test_cate_path)
    test_videos = test_data['video_path'].unique()
    concate_videos = test_data['ori'].unique()
    batch_size = 23
    ...
    test_output_txt_fd = open('bi_results7.txt', 'w')
